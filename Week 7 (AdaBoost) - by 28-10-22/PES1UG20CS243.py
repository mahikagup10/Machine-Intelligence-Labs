...
class AdaBoost:
    ...
    def stump_error(self, y, y_pred, sample_weights):
        ...
        # TODO
        err_wt = np.sum(sample_weights*np.where(y_pred!=y,1,0))
        sum_wt = np.sum(sample_weights)
        err = err_wt/sum_wt
    ...
    def update_weights(self, y, y_pred, sample_weights, alpha):
        ...
        a = self.stump_error(y, y_pred, sample_weights)
        if a == 0:
            return sample_weights
        for i in range(len(sample_weights)):
            if y_pred[i] == y[i]:
                sample_weights[i] = sample_weights[i] / (2 * (1-a))
            else:
                sample_weights[i] = sample_weights[i] / (2 * a)
        return sample_weights
