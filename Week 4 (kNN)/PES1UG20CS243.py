...
class KNN:
        ...
        for i in range(len(x)):
            ...
            for j in range(len(self.data)):
                ...
                for k in range(len(self.data[0])):
                    dist += abs((x[i][k] - self.data[j][k])) ** self.p

                final_dist = dist ** (1 / float(self.p))
                temp.append(final_dist)
    ...
    def k_neighbours(self, x):
        ...
        for d in self.find_distance(x):

            sorted_dist = list(enumerate(d))
            sorted_dist.sort(key=lambda l: l[1])
            
            idx_of_neigh = []
            neigh_dists = []

            for x in sorted_dist[0 : k]:

                idx_of_neigh.append(x[0])
                neigh_dists.append(x[1])

            index_of_neighbour.append(idx_of_neigh)
            neighbour_dist.append(neigh_dists)
        

        return (neighbour_dist, index_of_neighbour)
        pass
    ...
    def predict(self, x):
        ...
        if self.weighted==False:
            knn = KNeighborsClassifier(n_neighbors = self.k_neigh,p=self.p)
            knn.fit(self.data,self.target)
            pred=knn.predict(x)
        else:
            knn = KNeighborsClassifier(n_neighbors = self.k_neigh, p=self.p,weights='distance')
            knn.fit(self.data,self.target)
            pred=knn.predict(x)
        return pred
